...
// many numbers in the array are smaller than it. That is,
...
function smallerNumbersThanCurrent(nums) {
    ...
    function compareArrayByIndex(curIndex) {
        let smallerCount = 0;

        for (let i = 0; i < nums.length; i++) {
            if (curIndex !== i) {
                if (nums[curIndex] > nums[i]) smallerCount++;
            }
        }

        currentIndex++;
        smallerNumArr.push(smallerCount);

        if (smallerNumArr.length === nums.length) return;
        compareArrayByIndex(currentIndex);
    }
